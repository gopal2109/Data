...
var DeviceSearchableFields = map[string]string{
	"macAddress": "string",
	"type": "string",
	"deleted": "bool",
...
func NewDevice(devicetype string,
	...
	device.InventoryState = InventoryState{Id: bson.NewObjectId(), Device: device.Id}
...
func GetDevices(filters bson.M, offset, limit int) []Device {
	// type M map[string]interface{}
	var res []Device
	query := backend.GetDB().C("Devices").Find(filters)
...
func GetOneDevice(id string) (error, Device) {
	var res Device
	err := backend.GetDB().C("Devices").FindId(bson.ObjectIdHex(id)).One(&res)
	return err, res
}
