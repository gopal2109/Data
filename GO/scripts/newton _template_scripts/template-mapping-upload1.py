    ...
    def _return_columns_from_worksheet(self):
        ...
        for ws in self.wb:
            ...
            for row in ws.iter_rows():
                for cell in row:
                    if (cell.row == 1 and
                            cell.internal_value in self.column_list):
                        if cell.column not in self.column_letter_list:
                            cell_data = {}
                            cell_data['column'] = cell.column
                            cell_data['value'] = cell.internal_value
                ...
                for cell in row:
                    for sub_item in self.column_list:
                        ...
                        if (cell.column == column and
                                cell.internal_value is not None and
                                cell.row != 1):
                            try:
                                data = cell.internal_value.split(
                                    '-')[0].replace(' ', '')
                                desc = cell.internal_value.split(
                                    '-')[1]
                            except AttributeError:
                                data = str(int(cell.internal_value))
                            except IndexError:
                                desc = ''
                            if data != '':
                                key_dict[sub_item] = {}
